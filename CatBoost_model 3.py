...
for target in target_columns:
    ...
    # 构建 CatBoost 回归模型
    model = cb.CatBoostRegressor(random_seed=42, verbose=True)

    # 定义超参数搜索空间（自行调整）
    param_space = {
        'depth': (1, 6),
        'l2_leaf_reg': (3, 100),
        'learning_rate': (0.0001, 0.025),
        'iterations': (1, 1000),
        'random_strength': (1, 200),
        'bagging_temperature': (1, 200),
    }
    ...
    evaluation_metrics[target] = {'Train_R2': train_r2, 'Train_RMSE': train_rmse, 'Test_R2': test_r2,
                                  'Test_RMSE': test_rmse}
    hyperparameters[target] = best_params
