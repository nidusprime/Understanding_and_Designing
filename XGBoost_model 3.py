...
# 处理范围数据并取中值
def process_range_data(data):
    range_data = data['polymer MW (g/mol)'].str.split('-', expand=True)
    lower_limit = pd.to_numeric(range_data[0])
    upper_limit = pd.to_numeric(range_data[1])
    median_value = (lower_limit + upper_limit) / 2
    return median_value


data['polymer MW (g/mol)'] = process_range_data(data)

# 定义目标列
target_columns = ['mean pore radius (nm)', 'overall porosity (%)', 'contact angle (degree)']

# 对分类特征进行独热编码
categorical_features = ['polymer', ' polymer  SMILES string', 'pore maker ', 'additive', 'organic solvent']
